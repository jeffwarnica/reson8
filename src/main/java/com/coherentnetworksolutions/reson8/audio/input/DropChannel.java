 ...
 */
public class DropChannel implements InputChannel {
    ...
    public DropChannel(SignalEndpoint signalEndpoint, DropFactory dropFactory, Mixer mixer) {
    ...
    public void trigger(double volume) {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                DropInstance dropInstance = new DropInstance(cachedPcmData, volume);
    ...
    private class DropInstance {
        ...
        private Pad myReservedMixerPad; //this instances pad on the mixer
        ...
        public DropInstance(CachedWav cachedWav, double triggerVolume) {
            ...
            instanceSrc.connect((AppSrc.NEED_DATA) (elem, size) -> {
                ...
                int bytesPerFrame = cachedPcmData.bytesPerFrame(); // e.g., 3 for 24-bit mono, 4 for 16-bit stereo
                ...
                long currentTimestampNano = (long) ((bytesPushed / bytesPerFrame) * 1_000_000_000L / cachedPcmData.sampleRate());
                // buffer.setPresentationTimestamp(currentTimestampNano);

                long framesInThisBuffer = bufferSize / bytesPerFrame;
                long durationNano = (long) ((framesInThisBuffer * 1_000_000_000L) / cachedPcmData.sampleRate());
